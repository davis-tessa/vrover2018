...
#changed all f_dist from 30 to 50 for corner cases - circle at luna Park
...
def evade_rev_1(drive_time, drive_burst, mode):
    ...
    if front_dist > 150 and opt_dir == 'left':
        print("evade.evade_fwd_1  > Pivoting to the left")
        driveme_tank.pivot_left(drive_time, mode)
    ...
    elif front_dist > 150 and opt_dir == 'right':
        print("evade.evade_fwd_1  > Pivoting to the right")
        driveme_tank.pivot_right(drive_time, mode)
...
def check_bk_1(drive_time, drive_burst, mode):
...
def evade_fwd_3(drive_time, drive_burst, mode):
    drive_iterate = int(round(drive_time / drive_burst))
    ...
    if opt_dir == 'left':
        ...
        for y in range(drive_iterate):
            ##Run the function collision_avoidance() to check the distance from the front sensor to the closest object
            check_fr_4(drive_time, drive_burst, mode)
            ##Drive left and forward for drive_burst seconds
            driveme_tank.turn_left_fwd(drive_burst, mode)
    ##If the optimal direction is right, drive forward and right
    elif opt_dir == 'right':
...
def check_fr_3(drive_time, drive_burst, mode):
...
def evade_fwd_2(drive_time, drive_burst, mode):
    drive_iterate = int(round(drive_time / drive_burst))
    ...
    if opt_dir == 'left':
        ...
        for y in range(drive_iterate):
            ##Run the function collision_avoidance() to check the distance from the front sensor to the closest object
            check_fr_3(drive_time, drive_burst, mode)
            ##Drive left and forward for drive_burst seconds
            driveme_tank.turn_left_fwd(drive_burst, mode)
    ##If the optimal direction is right, drive forward and right
    elif opt_dir == 'right':
